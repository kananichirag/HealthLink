...
Object.defineProperty(exports, "DoctorService", {
    enumerable: true,
    get: function() {
...
let DoctorService = class DoctorService {
    async createPatient(dto, userId, tenantId) {
        ...
        if (dto.email) {
            ...
            if (existing) {
                throw new _common.ConflictException('A patient with this email already exists in your tenant');
            }
        }
        try {
    ...
    async createAllergyReport(dto, doctorId, tenantId) {
        ...
        if (!patient) {
            throw new _common.ForbiddenException('Patient does not belong to your tenant');
        }
    ...
    async createPrescription(dto, doctorId, tenantId) {
        ...
        const itemsWithMedicineIds = await Promise.all(dto.items.map(async (item)=>{
            ...
            if (!medicine) {
                throw new _common.BadRequestException(`Medicine "${item.medicineName}" not found in your tenant`);
            }
            return {
    ...
    async dispatchToPharmacy(prescriptionId, pharmacyId, doctorId, tenantId) {
        ...
        if (!connection) {
            throw new _common.BadRequestException('No active connection exists with the specified pharmacy');
        }
    ...
    async requestConnection(dto, doctorId, tenantId) {
        ...
        try {
            ...
            if (error instanceof _client.Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
                throw new _common.ConflictException('A connection with this pharmacy already exists');
            }
            throw error;
        }
    }
    ...
    async terminateConnection(connectionId, doctorId) {
        ...
        if (!connection) {
            throw new _common.NotFoundException('Connection not found');
        }
    ...
    async acceptConnection(connectionId, pharmacyId) {
        ...
        if (!connection) {
            throw new _common.NotFoundException('Pending connection not found');
        }
        const updated = await this.prisma.doctorPharmacyConnection.update({
            where: {
                id: connectionId
    ...
    async blockDate(dto, doctorId, tenantId) {
        ...
        try {
            ...
            if (error instanceof _client.Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
                throw new _common.ConflictException('This date is already blocked');
            }
            throw error;
        }
    }
    async unblockDate(date, doctorId, tenantId) {
        ...
        if (!blocked) {
            throw new _common.NotFoundException('Blocked date not found');
        }
    ...
    async cancelAppointment(appointmentId, doctorId, tenantId) {
        ...
        if (appointment.status === 'CANCELLED') {
            throw new _common.BadRequestException('Appointment is already cancelled');
        }
   ...
   */ async addRescheduledTag(appointmentId) {
    ...
    async completeAppointment(appointmentId, doctorId, tenantId) {
        ...
        if (appointment.status === 'COMPLETED') {
            throw new _common.BadRequestException('Appointment is already completed');
        }
        // Check if appointment is cancelled
        if (appointment.status === 'CANCELLED') {
            throw new _common.BadRequestException('Cannot complete a cancelled appointment');
        }
    ...
    constructor(prisma, notificationsService){
        ...
        this.logger = new _common.Logger(DoctorService.name);
    }
};
DoctorService = _ts_decorate([
