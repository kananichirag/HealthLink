...
Object.defineProperty(exports, "PharmacyService", {
    enumerable: true,
    get: function() {
...
const _salesservice = require("../sales/sales.service");
...
let PharmacyService = class PharmacyService {
    ...
    async dispensePrescription(prescriptionId, pharmacyUserId) {
        ...
        if (prescription.status === 'DISPENSED') {
            throw new _common.BadRequestException('Prescription is already dispensed');
        }
    ...
    async addMedicine(dto, tenantId) {
        ...
        try {
            ...
            if (error instanceof _client.Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
                throw new _common.ConflictException('A medicine with this batch number already exists');
            }
            this.logger.error(`Failed to add medicine: ${error.message}`);
            throw new _common.BadRequestException('Failed to add medicine');
        }
    }
    async updateMedicine(id, dto) {
        ...
        if (!existing) {
            throw new _common.NotFoundException('Medicine not found');
        }
        const data = {};
    ...
    async prescriptionCheckout(prescriptionId, pharmacyUserId) {
        ...
        const billItems = await Promise.all(prescription.items.map(async (item)=>{
            ...
            return {
                ...
                pricePerUnit: inventoryMatch ? Number(inventoryMatch.unitPrice ?? 0) : Number(item.medicine.unitPrice ?? 0),
    ...
    async sendBillToPatient(saleId) {
        ...
        if (!sale) {
            throw new _common.NotFoundException(`Sale ${saleId} not found`);
        }
        ...
        if (sale.prescription?.patient?.email) {
            const patientUser = await this.prisma.user.findFirst({
                where: {
                    email: sale.prescription.patient.email
                }
            });
            patientUserId = patientUser?.id ?? null;
        }
        if (!patientUserId) {
            throw new _common.NotFoundException('No patient user account found to send the bill to');
        }
        const itemsSummary = sale.items.map((item)=>`${item.medicine.name} x${item.quantity} - ${Number(item.totalPrice).toFixed(2)}`).join(', ');
    ...
    async getTopMedicines(tenantId, query) {
        ...
        const sorted = Array.from(medicineMap.values()).sort((a, b)=>b.totalQuantity - a.totalQuantity).slice(0, 10);
        return sorted;
    }
    ...
    constructor(prisma, notificationsService, salesService){
        ...
        this.logger = new _common.Logger(PharmacyService.name);
    }
};
PharmacyService = _ts_decorate([
